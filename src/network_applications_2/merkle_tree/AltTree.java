 ...
 * AltTree is an implementation of a Merkle binary hash tree where the leaves
 ...
 * AltTree knows how to serialize itself to a binary format, but does not
 ...
 * This AltTree is intentionally ignorant of the hashing/checksum algorithm
...
public class AltTree {
     ...
     * Use this constructor to create a AltTree from a list of leaf signatures.
    ...
    public AltTree(List<String> leafSignatures) throws NoSuchAlgorithmException {

        constructTree(leafSignatures);
        crc = MessageDigest.getInstance("SHA-256");
    ...
    public AltTree(Node treeRoot, int numNodes, int height, List<String> leafSignatures) throws NoSuchAlgorithmException {
    ...
    private static Node constructLeafNode(String signature) {
        ...
        byte[] buf = signature.getBytes();

        leaf.sig = digest != null ? digest.digest(buf) : new byte[0];
    ...
    static class Node {
        ...
        public String toString() {
            ...
            return String.format("AltTree.Node<type:%d, sig:%s, left (type): %s, right (type): %s>",
